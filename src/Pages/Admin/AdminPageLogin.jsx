...
import { Clock, Briefcase, Save, PlusCircle, CheckCircle, XCircle, Coffee, AlertCircle, MessageSquare, Paperclip, Users, TrendingUp } from 'lucide-react';
...
const UserPageLogin = () => {
  ...
  const getTodayWorkLogs = () => {
    return workLogs.filter(log => log.employeeId === currentEmployee.id && log.date === selectedDate);
  };
  ...
  const getTotalBreaks = () => {
    return getTodayWorkLogs().reduce((sum, log) => sum + (log.breakTime || 0), 0);
  };
  ...
  const getProductivity = () => {
    ...
    const highPriority = logs.filter(l => l.status === 'Completed' && l.priority === 'High').length;
      ...
      <div className="bg-white shadow-md border-b">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Employee Work Portal</h1>
              <p className="text-sm text-gray-600">Welcome, {currentEmployee.name}</p>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-800">{getTotalBreaks().toFixed(1)}h</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-800">{getCompleted()}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-800">{getProductivity()}%</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className={`px-6 py-3 rounded-lg font-medium transition ${
                activeView === 'today' ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100'
              }`}
            >
              Today's Work
            </button>
            <button
              ...
              className={`px-6 py-3 rounded-lg font-medium transition ${
                activeView === 'add' ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100'
              }`}
            >
              Add Entry
            </button>
          ...
          <div className="bg-white rounded-xl shadow-lg p-8">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewWorkLog({...newWorkLog, mood: e.target.value})}
                    className="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500"
                  >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewWorkLog({...newWorkLog, projectId: e.target.value})}
                    className="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500"
                  >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {['High', 'Medium', 'Low'].map(priority => (
                      <button
                        key={priority}
                        onClick={() => setNewWorkLog({...newWorkLog, priority})}
              ...
              <div className="grid grid-cols-4 gap-4">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Break</label>
                  <input
                    type="number"
                    value={newWorkLog.breakTime}
                    onChange={(e) => setNewWorkLog({...newWorkLog, breakTime: e.target.value})}
                    placeholder="0.5"
                    step="0.25"
                    className="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewWorkLog({...newWorkLog, hoursSpent: e.target.value})}
                    placeholder="8"
                    step="0.5"
                    className="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500"
                  />
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 h-20"
                />
              </div>
              ...
              <button
                onClick={addWorkLog}
                disabled={!newWorkLog.projectId || !newWorkLog.task || !newWorkLog.hoursSpent}
                  ...
                  <div key={log.id} className="bg-white rounded-xl shadow-lg p-6">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="text-lg font-bold">{getProjectName(log.projectId)}</h3>
                          <span className={`px-2 py-1 rounded text-xs font-bold ${
                            log.priority === 'High' ? 'bg-red-100 text-red-700' :
                            log.priority === 'Medium' ? 'bg-yellow-100 text-yellow-700' :
                      ...
                      <button onClick={() => deleteWorkLog(log.id)} className="text-red-500">
                        <XCircle className="w-5 h-5" />
                      </button>
                    </div>
                    <div className="flex gap-4 text-sm text-gray-600 mb-3">
                      <span>⏱️ {log.hoursSpent}h work</span>
                      {log.breakTime > 0 && <span>☕ {log.breakTime}h break</span>}
                    ...
                    {log.collaborators?.length > 0 && (
                      <div className="text-sm text-gray-600 mb-2">
                        👥 With: {log.collaborators.map(id => getMemberName(id)).join(', ')}
                      </div>
                    )}
                    {log.notes && (
